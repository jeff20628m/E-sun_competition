...
class Siamese_Loader:
    ...
    def get_batch(self, n):
        ...
        for i in range(n):
            ...
            pairs[0][i, :, :, :] = self.Xtrain[category,
                                               idx_1].reshape(self.w, self.h, 1)
            idx_2 = rng.randint(0, self.n_examples)
            # pick images of same class for 1st half, different for 2nd
            category_2 = category if i >= n//2 else (
                category + rng.randint(1, self.n_classes)) % self.n_classes
    ...
    def make_oneshot_task(self, N):
        ...
        test_image = np.asarray(
            [self.Xval[true_category, ex1, :, :]]*N).reshape(N, self.w, self.h, 1)
    ...
    def test_oneshot(self, model, N, k, verbose=0):
        ...
        if verbose:
            print(
                "Evaluating model on {} unique {} way one-shot learning tasks ...".format(k, N))
        for i in range(k):
        ...
        if verbose:
            print(
                "Got an average of {}% {} way one-shot learning accuracy".format(percent_correct, N))
...
def save_alphabets(basedir):
    ...
    for alphabet in os.listdir(base_dir):
        fig, ax = plot_alphabet(os.path.join(
            base_dir, alphabet), title=alphabet)
